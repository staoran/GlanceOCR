...
namespace OcrLiteLib
{
    public class OcrLite
    {
        ...
        public OcrResult Detect(Image img, int padding, int imgResize, float boxScoreThresh, float boxThresh, 
            float unClipRatio, bool doAngle, bool mostAngle)
        {
            return Detect(new Bitmap(img).ToMat(), padding, imgResize, boxScoreThresh, boxThresh, unClipRatio, doAngle, mostAngle);
        }

        public OcrResult Detect(Bitmap img, int padding, int imgResize, float boxScoreThresh, float boxThresh, 
            float unClipRatio, bool doAngle, bool mostAngle)
        {
            return Detect(img.ToMat(), padding, imgResize, boxScoreThresh, boxThresh, unClipRatio, doAngle, mostAngle);
        }

        public OcrResult Detect(Mat mat, int padding, int imgResize, float boxScoreThresh, float boxThresh, 
            float unClipRatio, bool doAngle, bool mostAngle)
        {
            Mat originSrc = new Mat();
            CvInvoke.CvtColor(mat, originSrc, ColorConversion.Bgr2Rgb);// convert to RGB
            Rectangle originRect = new Rectangle(padding, padding, originSrc.Cols, originSrc.Rows);
            Mat paddingSrc = OcrUtils.MakePadding(originSrc, padding);
